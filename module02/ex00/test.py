...
for a in ft_map(lambda dum: dum + 1, x):
    print(a, end=", ")
print("\nstd: ", map(lambda dum: dum + 1, x))
for a in map(lambda dum: dum + 1, x):
    print(a, end=", ")
# Output:
# <generator object ft_map at 0x7f708faab7b0> # The adress will be different
print("\nExample2\nft:  ", list(ft_map(lambda t: t + 1, x)))
...
print("\nExample4\nft:  ", list(ft_filter(lambda dum: not (dum % 2), x)))
...
for x in ft_map(None,None):
    print(x)
# print(list(map(function_to_apply = None, iterable = iterable)))
